...
class KnowledgeGraphEnhancedRAG:
    ...
    def __init__(
        ...
        self._load_kg_index()
        self.pg_query_engine = self.index.as_query_engine(
            include_text=True,
            llm=llm,
        )
    ...
    def _load_pgi_index(self) -> None:
        ...
        if not os.path.exists(PERSIST_DIR + "/pgi"):
            ...
            # persistent storage
            self.index.storage_context.persist(persist_dir=PERSIST_DIR + "/pgi")
    ...
    def _load_kg_index(self) -> None:
        ...
        if not os.path.exists(PERSIST_DIR + "/kg"):
            ...
            # persistent storage
            self.index.storage_context.persist(persist_dir=PERSIST_DIR + "/kg")
